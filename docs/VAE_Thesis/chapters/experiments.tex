...
\begin{figure}[ht]
    ...
    \caption{\ac{SSVAE} architecture used throughout all experiments. The network has $28*28=784$ inputs $\{x_i\}_{i=1}^{784}$ matching the \ac{MNIST} input, 256 hidden neurons $\{h_i\}_{i=1}^{256}$, 10 style variables $\{z_i\}_{i=1}^{10}$ and 10 digit neurons $\{y_i\}_{i=1}^{10}$. The decoder uses sampled $\{z'
    _i\}_{i=1}^{10}$ and $\{y'_i\}_{i=1}^{10}$ as inputs. Note that the dots between the neurons of a layer symbolize the remaining not shown neurons. Dashed lines symbolize the reparametrization trick and sampling. }
