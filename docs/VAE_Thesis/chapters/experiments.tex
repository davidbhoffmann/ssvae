...
Following previous work, our experiments where conducted with the \ac{MNIST} dataset \cite{deng2012mnist} using the digit label for partial supervision. We use the same fully connected feed forward architecture as \cite{narayanaswamy_learning_2017}, with \ac{ReLU} \cite{agarap2018deep} activations. The \ac{SSVAE} architecture used in the following experiments is detailed in \autoref{fig:architecture}.
...
The final test \ac{ELBO} remains relatively constant with respect to $\alpha$, which can also be seen in \autoref{apx:training}. A minor increase of the \ac{ELBO} can be observed wit increasing $\alpha$. The results further show that for the unsupervised $z$, stronger supervision decreases disentanglement. A table with detailed results can be found in \autoref{sec:results_alpha_exp}.
